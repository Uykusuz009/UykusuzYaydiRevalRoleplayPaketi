	...
	if exports.rl_integration:isPlayerSeniorAdmin(thePlayer) then
		...
		if id then
			...
			if (query) then
				...
				exports.rl_global:giveItem(thePlayer, 73, id)
...
function createElevatorWithFriend(thePlayer, commandName, targetPlayer)
	if (exports.rl_integration:isPlayerDeveloper(thePlayer) or exports.rl_integration:isPlayerLeaderAdmin(thePlayer) or exports.rl_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (targetPlayer) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Target Partial Nick or ID]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, tragetPlayerName = exports.rl_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local x, y, z = getElementPosition(thePlayer)
			local interiorwithin = getElementInterior(thePlayer)
			local dimensionwithin = getElementDimension(thePlayer)
...
function loadOneElevator(elevatorID, massLoad)
	thread:query("SELECT rot, tprot, id, x, y, z, tpx, tpy, tpz, dimensionwithin, interiorwithin, dimension, interior, car, disabled, oneway FROM `elevators` WHERE id = " .. elevatorID, function(res, rows, err)
		for index, row in ipairs(res) do
			...
			setElementData(elevatorElement, "entrance", {row.x, row.y, row.z, row.interiorwithin, row.dimensionwithin, row.rot, 0})
			setElementData(elevatorElement, "exit", {row.tpx, row.tpy, row.tpz, row.interior, row.dimension, row.tprot, 0})
			setElementData(elevatorElement, "status", {row.car, row.disabled == 1})
			setElementData(elevatorElement, "name",	row.name)
			...
			exports.rl_pool:allocateElement(elevatorElement, tonumber(row.id), true)
		end
	end)
	return true
end
...
function enterElevator(goingin)
	...
	elseif currentCP[INTERIOR_DIM] ~= 0 and otherCP[INTERIOR_DIM] ~= 0 and currentCP[INTERIOR_DIM] ~= otherCP[INTERIOR_DIM] then
	...
	local dbid, entrance, exit, interiorType, interiorElement = call(getResourceFromName("rl_interior"), "findProperty", player, otherCP[INTERIOR_DIM])
	if dbid > 0 then
	else
		dbid, entrance, exit, interiorType, interiorElement = call(getResourceFromName("rl_interior"), "findProperty", player, currentCP[INTERIOR_DIM])
	end
	...
	if vehicle then
		...
		if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
			setElementData(interiorElement, "lastused", exports.rl_datetime:now())
			dbExec(mysql:getConnection(),"UPDATE interiors SET lastused=NOW() WHERE id=" .. dbid)
			exports["rl_interior-manager"]:addInteriorLogs(dbid, "ENTERED/EXITED", player)
		end
	elseif isElement(player) then
		if movingInSameInt then
			...
			exports["rl_interior"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
		end
		return true
	else
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.rl_integration:isPlayerTrialAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
		if (not tonumber(intID) or tonumber(intID) % 1 ~= 0) and commandName ~= "PROPERTYCLEANUP" then
			...
			if tonumber(intID) == 0 and not exports.rl_integration:isPlayerSeniorAdmin(thePlayer) then
			...
			end, {thePlayer}, mysql:getConnection(), "SELECT `id` FROM `elevators` WHERE `dimensionwithin` = ? OR `dimension` = ?", intID, intID)
		end
	end
end
...
addEventHandler("toggleCarTeleportMode", root, function(player)
	...
	local query = dbExec(mysql:getConnection(), "UPDATE elevators SET car = ? WHERE id = ?", mode, getElementData(source, "dbid"))
	if query then
