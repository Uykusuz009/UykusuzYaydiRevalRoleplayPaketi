...
function requestInventory(button)
	if button=="left" and not getElementData(localPlayer, "exclusiveGUI") then
		...
		elseif type(getElementData(vehicle, "Impounded")) == "number" and isVehicleImpounded(vehicle) and not exports.rl_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then
...
function showVehicleMenu()
	...
	local vName = exports.rl_global:getVehicleName(vehicle)
	...
	if exports['rl_vehicle']:hasVehiclePlates(vehicle) and getElementData(vehicle, "show_plate") ~= 0 then
	...
	if (getPedSimplestTask(localPlayer) == "TASK_SIMPLE_CAR_DRIVE" and getPedOccupiedVehicle(localPlayer) == vehicle) or exports.rl_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (getElementData(localPlayer, "faction") > 0 and getElementData(localPlayer, "faction") == getElementData(vehicle, "faction")) then
		...
		if not isLocked or inCar then
			...
			if exports['rl_items']:hasItem(vehicle, 117) then
	...
	if vx < 0.05 and vy < 0.05 and vz < 0.05 and not getPedOccupiedVehicle(localPlayer) and not isVehicleLocked(vehicle) then -- completely stopped
		...
		if trailers[getElementModel(vehicle)] then
			if exports.rl_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then
				row.park = rightclick:addRow("Park")
				addEventHandler("onClientGUIClick", row.park, parkTrailer, false)
			end
		else
			if exports.rl_global:hasItem(localPlayer, 57) then -- FUEL CAN
	...
	if not isLocked then
		...
		elseif (getVehicleType(vehicle) == "Trailer" or getVehicleNameFromModel(608) == getVehicleName(vehicle)) then
		...
		elseif (getElementModel(vehicle) == 487 and getElementData(vehicle, "faction") == 2 or getElementModel(vehicle) == 417 and getElementData(vehicle, "faction") == 2) then
			row.stretcher = rightclick:addRow("Stretcher")
			addEventHandler("onClientGUIClick", row.stretcher, fStretcher, false)		
		end
	...
	if (exports.rl_integration:isPlayerTrialAdmin(localPlayer)) then
		if (exports.rl_global:isAdminOnDuty(localPlayer)) then
			...
			if (exports.rl_integration:isPlayerSeniorAdmin(localPlayer)) then
...
function lockUnlock(button, state)
	if (button=="left") then
		...
		elseif exports.rl_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (getElementData(localPlayer, "faction") > 0 and getElementData(localPlayer, "faction") == getElementData(vehicle, "faction")) then
...
function fillFuelTank(button, state)
	if (button=="left") then
		local _,_, value = exports.rl_global:hasItem(localPlayer, 57)
		if value > 0 then
