...
function createUniqueVehicle(data, existed)
	...
	local vehicle = exports.rl_pool:getElement("vehicle", tonumber(data.id))

	if not existed then
		local mQuery1 = dbExec(mysql:getConnection(), "INSERT INTO `vehicles_custom` SET `id`='"..(data["id"]).."', `brand`='"..(data["brand"]).."', `model`='"..(data["model"]).."', `year`='"..(data["year"]).."', `price`='"..(data["price"]).."', `tax`='"..(data["tax"]).."', `createdby`='"..(getElementData(client, "account_id")).."', `notes`='"..(data["note"]).."', `doortype` = " .. data.doortype)
		if not mQuery1 then
		...
		exports.rl_global:sendMessageToAdmins("[ADM] "..getElementData(client, "account_username").." has created new unique vehicle #"..data.id..".")
		exports.rl_vehicle:reloadVehicle(tonumber(data.id))
		return true
	else
		local mQuery1 = dbExec(mysql:getConnection(), "UPDATE `vehicles_custom` SET `brand`='"..(data["brand"]).."', `model`='"..(data["model"]).."', `year`='"..(data["year"]).."', `price`='"..(data["price"]).."', `tax`='"..(data["tax"]).."', `updatedby`='"..(getElementData(client, "account_id")).."', `notes`='"..(data["note"]).."', `updatedate`=NOW(), `doortype` = " .. data.doortype .. "  WHERE `id`='"..(data["id"]).."' ")
		if not mQuery1 then
			outputDebugString("VEHICLE MANAGER / VEHICLE LIB / createUniqueVehicle / DATABASE ERROR")
		...
		exports.rl_global:sendMessageToAdmins("[ADM] "..getElementData(client, "account_username").." has updated unique vehicle #"..data.id..".")
		exports.rl_vehicle:reloadVehicle(tonumber(data.id))
...
function resetUniqueVehicle(vehID)
	if exports.rl_integration:isPlayerManager(client) then	
		...
		exports.rl_global:sendMessageToAdmins("[ADM] "..getElementData(client, "account_username").." has removed unique vehicle #"..vehID..".")
