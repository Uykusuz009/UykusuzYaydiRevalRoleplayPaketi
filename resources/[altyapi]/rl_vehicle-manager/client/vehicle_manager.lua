...
function createVehManagerWindow(data)
    ...
    local lTotal = guiCreateLabel(460, 25, 164, 17, "Standart Araçlar: " .. exports.rl_global:formatMoney(#standardVehs), false, gWin)
    local lActive = guiCreateLabel(460, 42, 164, 17, "Benzersiz Araçlar: " .. exports.rl_global:formatMoney(#uniqueVehs), false, gWin)
    ...
    function fetchSearchResults(interiorsResultList)
        if interiorsResultList then
            ...
            for _, record in ipairs(interiorsResultList) do
                ...
                -- guiGridListSetItemText(targetList[i], row, colOwner[i], exports.rl_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Bilinmiyor", false, false)
...
function loadTab(vehs, i)
    for index, veh in ipairs(vehs) do
        ...
        guiGridListSetItemText(targetList[i], row, colName[i], exports.rl_global:getVehicleName(veh), false, false)
...
function formatLastUsed(sec)
    if sec or not tonumber(sec) then
        ...
        return exports.rl_datetime:formatTimeInterval(sec) .. " önce"
    end
end
...
function createCheckVehWindow(result, adminTitle, history, notes1)
    ...
    guiSetInputEnabled(true)
    notes = notes1
    checkVehWindow = guiCreateWindow(sx / 2 - 300, sy / 2 - 233, 600, 466, "Vehicle Manager - Vehicle Admin Note - " .. adminTitle .. " " .. getElementData(localPlayer, "account_username"), false)
    ...
    guiSetText(lVehModelID, "Vehicle Name/ID: " ..(getVehicleNameFromModel(result[1][2]) or "Bilinmiyor") .. " (ID #" ..(result[1][1] or "Bilinmiyor") .. ")")
    ...
    guiSetText(lCarHP, "HP: " .. math.floor(tonumber(result[1][8]) / 10) .. "% (" .. math.floor(tonumber(result[1][8])) .. ")" .. "     " .. "Fuel: " ..(result[1][6] .. "%" or "Bilinmiyor") .. "     " .. "Paintjob: " ..(result[1][7] == "0" and "None" or result[1][7]))
    ...
    guiSetText(lCreateDate, "Created Date: " ..(result[1][25] or "Bilinmiyor"))
    ...
    if result[1][21] then
        ...
        addEventHandler("onClientGUIClick", bRestoreVeh1, function(button)
            if button == "left" then
                if triggerServerEvent("vehicleManager:restoreVeh", localPlayer, result[1][1]) then
        ...
        addEventHandler("onClientGUIClick", bRemoveVeh1, function(button)
            if button == "left" then
                if triggerServerEvent("vehicleManager:removeVeh", localPlayer, result[1][1]) then
                    closeCheckVehWindow()
                end
            end
        end, false)
        guiSetEnabled(bRemoveVeh1, exports.rl_integration:isPlayerSeniorAdmin(localPlayer))
    else
        ...
        addEventHandler("onClientGUIClick", bGotoVeh1, function(button)
            if button == "left" then
                triggerServerEvent("vehicleManager:gotoVeh", localPlayer, result[1][1])
            end
        end, false)
        ...
        addEventHandler("onClientGUIClick", bDelVeh1, function(button)
            if button == "left" then
                if triggerServerEvent("vehicleManager:delVeh", localPlayer, result[1][1]) then
                    triggerServerEvent("vehicleManager:checkveh", localPlayer, localPlayer, "checkveh", result[1][1])
                end
            end
        end, false)
    ...
    addEventHandler("onClientGUIClick", bSave, function(button)
        if button == "left" then
            ...
            if noteMemo or(currentNote and currentNote.edit and currentNote.note ~= noteMemo) then
                ...
                exports.rl_global:playSoundSuccess()
                closeCheckVehWindow()
            else
