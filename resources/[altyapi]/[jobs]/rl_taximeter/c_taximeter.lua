...
function showTaximeterGui()
	...
	if isDriver then
		gui_base = guiCreateStaticImage(x,y,width,height,"images/base_driver.png",false,nil)
	else
		gui_base = guiCreateStaticImage(x,y,width,height,"images/base_pax.png",false,nil)
	end
	...
	if isDriver then --buttons etc. go here
		gui_btn1 = guiCreateStaticImage(24,112,31,31,"images/btn.png",false,gui_base)
			addEventHandler("onClientGUIClick", gui_btn1, toggleMeter)
			if meterRunning then
				guiStaticImageLoadImage(gui_btn1, "images/btn_active.png")
			end
		
		gui_btn2 = guiCreateStaticImage(82,112,31,31,"images/btn.png",false,gui_base) --x=+27 =58
			addEventHandler("onClientGUIClick", gui_btn2, resetMeter)
		
		gui_btn3 = guiCreateStaticImage(140,112,31,31,"images/btn.png",false,gui_base)
			addEventHandler("onClientGUIClick", gui_btn3, setFare)
		
		gui_btn4 = guiCreateStaticImage(198,112,31,31,"images/btn.png",false,gui_base)
			addEventHandler("onClientGUIClick", gui_btn4, toggleLight)
			if lightOn then
				guiStaticImageLoadImage(gui_btn4, "images/btn_on.png")
			end		
	else
		if meterRunning then
	...
	if(source == gui_btn1) then
		if isDriver then
			if meterRunning then
				...
				guiStaticImageLoadImage(gui_btn1, "images/btn.png")
			else
				meterRunning = true
				local theVehicle = getPedOccupiedVehicle(localPlayer)
...
function toggleLight()
	if(source == gui_btn4) then
		if lightOn then --turn off
			...
			guiStaticImageLoadImage(gui_btn4, "images/btn.png")
		else --turn on
			lightOn = true
			guiStaticImageLoadImage(gui_btn4, "images/btn_on.png")
			triggerServerEvent("taximeter:setLight", localPlayer, lightOn)
