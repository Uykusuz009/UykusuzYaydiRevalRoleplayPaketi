...
function startListening(player)
	if not exports.rl_integration:isPlayerDeveloper(player) then
		outputChatBox("[!]#FFFFFF Yeterli yetkiniz yok.", player, 255, 0, 0, true)
...
addCommandHandler("ipb", startListening, false, false)

function changeCategory(newCategory)
	if not exports.rl_integration:isPlayerDeveloper(client) then
		return
	end
