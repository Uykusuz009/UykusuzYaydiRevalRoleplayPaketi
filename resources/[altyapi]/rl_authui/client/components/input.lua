...
Input.render = function(options, store)
	...
	local colorTheme = options.colorTheme
    ...
    if hover then
        exports.rl_cursor:setCursor("all", "ibeam")
	...
	if variant == 'rounded' then
		drawRoundedRectangle({
			...
			color = COLORS_BY_NAME[Input.defaultColor][currentPalette.background],
    ...
    if not value or value == "" then
        if placeholder then
            dxDrawText(placeholder,
                    ...
                    position.y,
                    0,
                    size.y + position.y, colorTheme or
    ...
    if isInputFocused then
        ...
        dxDrawLine(cursorPosition + gap + 2,
                ...
                position.y + size.y - (size.y / 4), colorTheme or
