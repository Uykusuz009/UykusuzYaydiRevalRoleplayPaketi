...
local function canFly()
	if getResourceState(getResourceFromName("rl_integration")) == "running" then
		return (exports.rl_integration:isPlayerTrialAdmin(localPlayer) and getElementData(localPlayer, "duty_admin") and exports.rl_global:isAdminOnDuty(localPlayer) or getElementData(localPlayer, "canFly") and not getElementData(localPlayer, "parachuting") and not getElementData(localPlayer, "parachuting"))
	else
		return false
	end
end
