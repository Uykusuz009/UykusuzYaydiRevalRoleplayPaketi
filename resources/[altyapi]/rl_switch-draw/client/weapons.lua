...
function traceBullet(weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement)
	if not (weapon == 39 or weapon == 40) then
		if source == localPlayer and not weapon_infinite_ammo[weapon] then
			...
			if not can then
				if why == 'Mermi kalmadı!' then
					playSound("public/no_ammo.mp3")
				else
					outputChatBox("[!]#FFFFFF " .. why, 255, 0, 0, true)
					playSoundFrontEnd(4)
				end
			end
		end
...
function weaponInteract(item)
	local item_values = exports.rl_global:explode(":", item[2])
	local serial_info = exports.rl_global:retrieveWeaponDetails(item_values[2])
	local gun_source = tonumber(serial_info[2])

	if gun_source == 2 then
		outputChatBox("Görev silahını değiştiremezsiniz.", 255, 0, 0)
		playSoundFrontEnd(4)
	else
		closeWeaponInteract()
        GUIEditor.window[1] = guiCreateWindow(677, 403, 177, 105, item_values[3] or "Belirsiz Silah" , false)
        ...
        guiSetEnabled(GUIEditor.button[2], gun_source ~= 2 and (weapon_ammoless[tonumber(item_values[1])]~=nil or exports.rl_integration:isPlayerTrialAdmin(localPlayer, true) or exports.rl_integration:isPlayerHelper(localPlayer, true) or not item_values[5] or (item_values[5] and item_values[5]~="1")))
        	...
        	elseif source == GUIEditor.button[1] then
				...
				if isGun(id) and not weapon_ammoless[id] then
					...
					if loaded_ammo and loaded_ammo > 0 then
						if reloading then
							playSoundFrontEnd(4)
							outputChatBox("Lütfen bekleyin...", 255,0,0)
						else
							reloading = true
							syncAmmo()
        					closeWeaponInteract()
        				...
        				playSoundFrontEnd(4)
						outputChatBox("Senin " .. item_values[3] .. " itemin boş.", 255,0,0)
        			end
        		else
        			playSoundFrontEnd(4)
					outputChatBox("Bu silah mermi kullanmıyor!", 255,0,0)
        		end
        	end
        end)
