...
benzincol = createColSphere( 1944.234375, -1775.49609375, 13.39999961853, 3 )
...
if not (id) then
		  ...
		  elseif benzinmiktar <= 0 then
		   outputChatBox("#FF0000[!]#f6f6f6 Miktar sıfır veya sıfırdan küçük olamaz.",thePlayer, 255, 255, 255, true)
		   else
		   local veh = getPedOccupiedVehicle(thePlayer)
		  ...
		  elseif cbenzin >=  100 then 
		  	outputChatBox("#FF0000[!]#f6f6f6 Aracınızın benzin deposu zaten dolu.",thePlayer, 255, 255, 255, true)
		   else
		if isPedInVehicle(thePlayer) then
			local veh = getPedOccupiedVehicle(thePlayer)
			if (veh) then
...
local para = exports.rl_global:getMoney(thePlayer)
 ...
 exports.rl_global:takeMoney(thePlayer, id * 2)
