...
function showFactionList(factions)
	if not (wFactionList) then
		wFactionList = guiCreateWindow(0.25, 0.25, 0.5, 0.5, "Birlikler", true)
		...
		local colName = guiGridListAddColumn(gridFactions, "Adı", 0.6)
		...
		addEventHandler("onClientGUIDoubleClick", gridFactions, function(button)
			...
			if row ~= -1 and col ~= -1 then
				...
				elseif button == "right" then
					triggerServerEvent("faction:admin:showf3", localPlayer, gridID, exports.rl_integration:isPlayerAdmin1(localPlayer))
				end
			else
				outputChatBox("[!]#FFFFFF Birlik seçin.", 255, 0, 0, true)
				playSoundFrontEnd(4)
			end
		end, false)

		bFactionListClose = guiCreateButton(0.025, 0.9, 0.95, 0.1, "Kapat", true, wFactionList)
...
function createAdminDuty(factionT, dutyT)
    ...
    exports.rl_global:guiComboBoxAdjustHeight(DutyAllow.combobox[1], #factionT)
    ...
    exports.rl_global:guiComboBoxAdjustHeight(DutyAllow.combobox[2], 2)
	...
	if selection == 0 then
		for k,v in pairs(factionTable[key][3]) do
			if tonumber(v[2]) > 0 then
				...
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 2, exports["rl_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 3, exports["rl_items"]:getItemDescription(v[2], v[3]), false, false)
...
function allowItem()
	...
	if not exports['rl_items']:isItem(itemID) and selection == 0 then
