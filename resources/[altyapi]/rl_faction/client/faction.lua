...
addEventHandler("birlik:panel",root,function(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikSeviye,birlikonay)
	if not gFactionWindow2 then
		...
		zaman = setTimer(function()
			...
			showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikSeviye,birlikonay)
		end, 1000, 1)
	end
end)
...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, birlikSeviye,birlikonay)
	if (gFactionWindow==nil) then
		...
		colName = guiGridListAddColumn(gMemberGrid, "Ad", 0.20)
		...
		for k, v in ipairs(memberUsernames) do
			...
			if (memberOnline[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colOnline, "İn-Aktif", false, false)
		...
		guiSetText(gFactionWindow, tostring(teamName) .. " (" .. counterOnline .. "/" .. (counterOnline + counterOffline) .. " Aktif Üyeler)")
		...
		if (localPlayerIsLeader) then
			...
			gButtonPromote = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Üyeyi Tanıt", true, tabOverview)
			gButtonEditRanks = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Rütbeleri Düzenle", true, tabOverview)
			...
			gButtonRespawnui = guiCreateButton(0.825, 0.5356, 0.16, 0.06, "Tüm Araçları Yenile", true, tabOverview)
			...
			colVehLocation = guiGridListAddColumn(gVehicleGrid, "Bölge", 0.4)
			gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Arabayı Yenile", true, tabVehicles)
			...
			if towstats then
				...
				local colName = guiGridListAddColumn(gTowGrid, "Ad", 0.2)
		...
		gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Birlikden Ayrıl", true, tabOverview)
		gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Kapat", true, tabOverview)
...
function showrespawn()
	...
	showrespawnUI = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Aracın Yenilenmesi", false)
...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		wRanks = guiCreateWindow(x, y, width, height, "Rütbeler ve Ücretler", false)
		
		local y = 0
		for i=1, 20 do
			y = (y or 0) + 23
			lRanks[i] = guiCreateLabel(0.05 * width, y + 3, 0.4 * width, 20, "Rütbe #" .. i .. " Başlık ve Ödeme: ", false, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("[!]#FFFFFF Diziniz geçersiz karakterler içeriyor, lütfen bunları '@.; gibi karakterler içermediğinden emin olun.", 255, 0, 0, true)
			playSoundFrontEnd(4)
		elseif not (isNumber) then
			outputChatBox("[!]#FFFFFF Ödemeleriniz rakam değildir, lütfen rakam girdiğinizden ve para birimi simgesi girmediğinizden emin olun.", 255, 0, 0, true)
			playSoundFrontEnd(4)
		else
			local sendRanks = {}
			local sendWages = {}
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "MOTD", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)
			
			guiSetInputEnabled(true)
			
			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Yenile", true, wMOTD)
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "Hiç oyuncu bulunamadı veya az sayıda oyuncu bulundu.", true, wInvite)
...
function checkNameExists(theEditBox)
	...
	if (count>1) then
		guiSetText(lNameCheck, "Çok fazla oyuncu bulundu.")
	...
	elseif (count==0) then
		guiSetText(lNameCheck, "Hiç oyuncu bulunamadı veya az sayıda oyuncu bulundu.")
		guiLabelSetColor(lNameCheck, 255, 0, 0)
		guiMemoSetReadOnly(tInvite, true)
		guiSetEnabled(bInvite, false)
	end
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Gerçekten ayrılmak istiyor musunuz?",true,wConfirmQuit)
		guiLabelSetHorizontalAlign (lQuestion,"center",true)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		if (playerName~="") then
			...
			outputChatBox("[!]#FFFFFF " .. playerName:gsub("_", " ") .. " isimli oyuncuyu birlikden attınız.", 0, 255, 0, true)
			triggerServerEvent("cguiKickPlayer", localPlayer, playerName)
		else
			outputChatBox("[!]#FFFFFF Lütfen atılacak üyeyi seçin.", 255, 0, 0, true)
			playSoundFrontEnd(4)
		end
	end
end
...
function btButtonPerk(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPerk) then
		...
		if (playerName == "") then
			outputChatBox("[!]#FFFFFF Lütfen düzenlenecek üyeyi seçin.", 255, 0, 0, true)
			playSoundFrontEnd(4)
			return
		end
...
function btRespawnOneVehicle(button, state)
	if button == "left" and state == "up" then
		...
		if vehID then
			...
			outputChatBox("[!]#FFFFFF Lütfen yenilemek için bir araç seçin.", 255, 0, 0, true)
			playSoundFrontEnd(4)
		end
	end
end
...
function btToggleLeader(button, state)
	if (button=="left") and (state=="up") and (source==gButtonLeader) then
		...
		elseif (playerName~="") then
			...
			if (currentLevel=="Lider") then
				guiGridListSetItemText(gMemberGrid, row, tonumber(colLeader), "Üye", false, false)
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
				triggerServerEvent("cguiToggleLeader", localPlayer, playerName, false)
			else
				guiGridListSetItemText(gMemberGrid, row, tonumber(colLeader), "Lider", false, false)
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
				triggerServerEvent("cguiToggleLeader", localPlayer, playerName, true)
			end
		else
			outputChatBox("[!]#FFFFFF Lider arasında geçiş yapmak için lütfen bir üye seçin.", 255, 0, 0, true)
			playSoundFrontEnd(4)
		end
	end
end
...
function btPromotePlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPromote) then
		...
		if (playerName~="") then
			...
			promotionWindow[1] = guiCreateWindow(0.3887,0.2383,0.1713,0.5834,"Üyeyi Tanıt",true)
			...
			promotionRadio[20] = guiCreateRadioButton(0.047,0.1071,0.9145,0.0402,"Dinamik Rütbe 20",true,promotionWindow[1])
			promotionRadio[19] = guiCreateRadioButton(0.047,0.1473,0.9145,0.0402,"Dinamik Rütbe 19",true,promotionWindow[1])
			promotionRadio[18] = guiCreateRadioButton(0.047,0.1875,0.9145,0.0402,"Dinamik Rütbe 18",true,promotionWindow[1])
			promotionRadio[17] = guiCreateRadioButton(0.047,0.2277,0.9145,0.0402,"Dinamik Rütbe 17",true,promotionWindow[1])
			promotionRadio[16] = guiCreateRadioButton(0.047,0.2679,0.9145,0.0402,"Dinamik Rütbe 16",true,promotionWindow[1])
			promotionRadio[15] = guiCreateRadioButton(0.047,0.308,0.9145,0.0402,"Dinamik Rütbe 15",true,promotionWindow[1])
			promotionRadio[14] = guiCreateRadioButton(0.047,0.3482,0.9145,0.0402,"Dinamik Rütbe 14",true,promotionWindow[1])
			promotionRadio[13] = guiCreateRadioButton(0.047,0.3884,0.9145,0.0402,"Dinamik Rütbe 13",true,promotionWindow[1])
			promotionRadio[12] = guiCreateRadioButton(0.047,0.4286,0.9145,0.0402,"Dinamik Rütbe 12",true,promotionWindow[1])
			promotionRadio[11] = guiCreateRadioButton(0.047,0.4688,0.9145,0.0402,"Dinamik Rütbe 11",true,promotionWindow[1])
			promotionRadio[10] = guiCreateRadioButton(0.047,0.5089,0.9145,0.0402,"Dinamik Rütbe 10",true,promotionWindow[1])
			promotionRadio[9] = guiCreateRadioButton(0.047,0.5491,0.9145,0.0402,"Dinamik Rütbe 9",true,promotionWindow[1])
			promotionRadio[8] = guiCreateRadioButton(0.047,0.5893,0.9145,0.0402,"Dinamik Rütbe 8",true,promotionWindow[1])
			promotionRadio[7] = guiCreateRadioButton(0.047,0.6295,0.9145,0.0402,"Dinamik Rütbe 7",true,promotionWindow[1])
			promotionRadio[6] = guiCreateRadioButton(0.047,0.6696,0.9145,0.0402,"Dinamik Rütbe 6",true,promotionWindow[1])
			promotionRadio[5] = guiCreateRadioButton(0.047,0.7098,0.9145,0.0402,"Dinamik Rütbe 5",true,promotionWindow[1])
			promotionRadio[4] = guiCreateRadioButton(0.047,0.75,0.9145,0.0402,"Dinamik Rütbe 4",true,promotionWindow[1])
			promotionRadio[3] = guiCreateRadioButton(0.047,0.7902,0.9145,0.0402,"Dinamik Rütbe 3",true,promotionWindow[1])
			promotionRadio[2] = guiCreateRadioButton(0.047,0.8304,0.9145,0.0402,"Dinamik Rütbe 2",true,promotionWindow[1])
			promotionRadio[1] = guiCreateRadioButton(0.047,0.8705,0.9145,0.0402,"Dinamik Rütbe 1",true,promotionWindow[1])
			promotionButton[1] = guiCreateButton(0.0427,0.9107,0.4231,0.067,"Kaydet",true,promotionWindow[1])
			promotionButton[2] = guiCreateButton(0.5214,0.9107,0.4231,0.067,"Kapat",true,promotionWindow[1])
...
function hideFactionMenu()
	...
	gFactionWindow, gMemberGrid, gMOTDLabel, colName, colRank, colWage, colLastLogin, colLeader, colOnline, gButtonKick, gButtonPromote, gButtonDemote, gButtonEditRanks, gButtonEditMOTD, gButtonInvite, gButtonLeader, gButtonQuit, gButtonExit = nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil
	theMotd, theTeam, arrUsernames, arrRanks, arrLeaders, arrOnline, arrFactionRanks, arrFactionWages, arrLastLogin, membersOnline, membersOffline = nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil
...
function loadFinance()
	if source == tabFinance then
		if not financeLoaded then	
			local label = guiCreateLabel(0,0,1,1,"Yükleniyor...",true,tabFinance)
...
function fillFinance(factionID, bankThisWeek, bankPrevWeek, bankmoney, vehiclesvalue, propertiesvalue)
	...
	guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.rl_global:formatMoney(bankmoney)), false, false)
	local row = guiGridListAddRow(assetsGridlist)
	guiGridListSetItemText(assetsGridlist, row, 1, "Araç", false, false)
	if not vehiclesvalue then vehiclesvalue = 0 end
	guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.rl_global:formatMoney(vehiclesvalue)), false, false)
	local row = guiGridListAddRow(assetsGridlist)
	guiGridListSetItemText(assetsGridlist, row, 1, "Mülker", false, false)
	if not propertiesvalue then propertiesvalue = 0 end
	guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.rl_global:formatMoney(propertiesvalue)), false, false)

	local row = guiGridListAddRow(assetsGridlist)
	guiGridListSetItemText(assetsGridlist, row, 1, "Toplam", false, false)
	guiGridListSetItemText(assetsGridlist, row, 2, "$" .. tostring(exports.rl_global:formatMoney(bankmoney+vehiclesvalue+propertiesvalue)), false, false)
	...
	for k,v in ipairs(insertStatement) do
		...
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColLast, "$" .. tostring(exports.rl_global:formatMoney(lastWeek[rowVar])), false, false)
		guiGridListSetItemText(weeklyStatementGridlist, row, statementColThis, "$" .. tostring(exports.rl_global:formatMoney(thisWeek[rowVar])), false, false)
		...
		elseif transTypes then
			...
			for k,v in ipairs(bankThisWeek) do
				if transTypes[v.type] then
					...
					guiGridListSetItemText(transactionDetailsGridlist, row, 5, tostring(exports.rl_global:formatMoney(v.amount)), false, false)
					if v.amount > 0 then
	...
	for k,v in ipairs(bankThisWeek) do
		...
		guiGridListSetItemText(transactionsGridlist, row, 5, tostring(exports.rl_global:formatMoney(v.amount)), false, false)
		if v.amount > 0 then
	...
	if customEditID ~= 0 then
		...
		for k,v in pairs(customg[customEditID][5]) do
			if tonumber(v[2]) >= 0 then -- Items
				...
				guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["rl_items"]:getItemName(v[2]), false, false) -- Item Name
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["rl_items"]:getItemName(v[2]), false, false) -- Weapon Name
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, tostring(v[3]), false, false) -- Ammo
					guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, { v[1], tonumber(v[2]), v[3], v[4] })
				end
	...
	for k,v in pairs(allowList) do
		if tonumber(v[2]) >= 0 then -- Items
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["rl_items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["rl_items"]:getItemDescription(v[2], v[3]), false, false)
			...
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				if tonumber(v[2]) == -100 then
					...
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["rl_items"]:getItemName(v[2]), false, false)
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, v[3], false, false)
					guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, { v[1], tonumber(v[2]), v[3] })
				end
			end
...
function addDutyItem()
	...
	if r >= 0 then
		...
		guiGridListSetItemText(DutyCreate.gridlist[4], row, 1, exports["rl_items"]:getItemName(info[2]), false, false) -- Item Name
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			if tonumber(info[1]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, exports["rl_items"]:getItemName(info[2]), false, false)
				guiGridListSetItemText(DutyCreate.gridlist[1], row, 2, tostring(info[4]), false, false)
				guiGridListSetItemData(DutyCreate.gridlist[1], row, 1, info)
			end
		end
...
function removeDutyItem()
	...
	if r >= 0 then
		...
		if red == 255 and g ~= 0 and b ~= 0 then
			...
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 1, exports["rl_items"]:getItemName(tonumber(info[2])), false, false)
			guiGridListSetItemText(DutyCreate.gridlist[2], row, 2, exports["rl_items"]:getItemDescription(tonumber(info[2]), info[3]), false, false)
...
function createDuty()
   ...
   --guiGridListAddColumn(DutyCreate.gridlist[2], "ID", 0.1)
    ...
    addEventHandler("onClientGUIClick", DutyCreate.button[3], function()
		...
		if r >= 0 then
			...
			if tonumber(info[2]) == -100 then
				...
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, exports["rl_items"]:getItemName(tonumber(info[2])), false, false) -- Weapon Name
				guiGridListSetItemData(DutyCreate.gridlist[3], row, 2, info)
				guiGridListSetItemText(DutyCreate.gridlist[3], row, 2, ammo, false, false) -- Ammo
			end
    ...
    DutyCreate.button[8] = guiCreateButton(160, 512, 138, 44, "Bölge", false, DutyCreate.window[1])
...
function createLocations()
    ...
    guiGridListAddColumn(DutyLocations.gridlist[1], "Ad", 0.9)
    ...
    guiGridListAddColumn(DutyLocations.gridlist[2], "Ad", 0.9)
