...
function invite(player, cmd, arg, betAmount)
	if not (arg == "kabul" or arg == "ret") then
        if not (arg and betAmount) then
            outputChatBox("[!] #FFFFFFKULLANIM: /davet <oyuncu id> <Bahis Miktarı>", player, 0, 55, 255, true)
            outputChatBox("[!] #FFFFFFKULLANIM: /davet kabul/ret", player, 0, 55, 255, true)
            return
        end
    end
    if tonumber(arg) then
        local invitedPlayer = exports.rl_global:findPlayerByPartialNick(player, arg)
        ...
        if invitedPlayer == player then
            outputChatBox("[!] #FFFFFFKendine davet gönderemezsin.", player, 255, 0, 0, true)
            return
        end	
		
		if getElementInterior(player) ~= 12 or getElementDimension(player) ~= 102 then			
			outputChatBox("[!] #FFFFFFOyuncu davet etmek için kumarhanede olmanız gerekmektedir.", player, 255, 0, 0, true)
			return
		end

		if getElementInterior(invitedPlayer) ~= 12 or getElementDimension(invitedPlayer) ~= 102 then			
			outputChatBox("[!] #FFFFFFBu oyuncu sizin ile aynı bölgede değil.", player, 255, 0, 0, true)
			return
		end
        
		if not exports.rl_global:hasMoney(player, betAmount) then
             outputChatBox("[!] #FFFFFFBahis yaptığınız kadar para üzerinizde yok.", player, 255, 0, 0, true)
            return
        end
		
		if not exports.rl_global:hasMoney(invitedPlayer, betAmount) then
        ...
        triggerClientEvent(invitedPlayer, "Karazindan:pairs:inviteGUI", invitedPlayer, getPlayerName(player), betAmount)
    ...
    elseif arg == "kabul" then
        ...
        if (isElement(inviterPlayer)) then
            ...
            triggerClientEvent(inviterPlayer, 'Karazindan:pairs:showGUI', inviterPlayer, getPlayerName(player), betAmount)
            triggerClientEvent(player, 'Karazindan:pairs:showGUI', player, getPlayerName(inviterPlayer), betAmount)
        else 
...
function spin(player)
    ...
    if not exports.rl_global:hasMoney(player, getElementData(player, "betAmount")) then
    ...
    for i, player in ipairs({player, opponent}) do
        ...
        if exports.rl_global:hasMoney(player, getElementData(player,"betAmount")) then
            exports.rl_global:takeMoney(player,getElementData(player,"betAmount"))
        else
        ...
        if winner then
            ...
            if winner == player then
                exports.rl_global:giveMoney(winner,getElementData(player,"betAmount")*2)
            end
        else
            outputChatBox("[!] #FFFFFFBu raundu kimse kazanamadı (berabere)", player, 0, 55, 255, true)
            exports.rl_global:giveMoney(player, getElementData(player, "betAmount"))
        end
        ...
        if exports.rl_global:hasMoney(player, getElementData(player, "betAmount")) then
            --exports.rl_global:takeMoney(player, getElementData(player, "betAmount"))
            triggerClientEvent(player, 'Karazindan:pairs:showGUI', player, getPlayerName(opponent), getElementData(player,"betAmount"))
        else
