...
function createBlur(name, strenght, extra,alpha)
    ...
    if #blurShaders >= 1 then
        if not isRenderingShader then
			...
			duration = alpha
            addEventHandler("onClientPreRender", root, renderBlur)
        end
    end
end
		...
		if v.s_name == name then
            ...
            if #blurShaders <= 0 then
                if isRenderingShader then
                    ...
                    removeEventHandler("onClientPreRender", root, renderBlur)
                end
            end
			return
		end
...
function renderBlur()
    for k, v in ipairs(blurShaders) do
        if v.shader then
            ...
            now = getTickCount()
            alpha = interpolateBetween(0, 0, 0, v.strenght_s, 0, 0, (now - startTime) / duration, "Linear")
            dxSetShaderValue(v.shader, "ScreenSource", myScreenSource)
            dxSetShaderValue(v.shader, "UVSize", screenWidth, screenHeight)
            dxSetShaderValue(v.shader, "BlurStrength", alpha)
            ...
            dxDrawImageSection(x, y, w, h, x, y, w, h, v.shader, 0, 0, 0, tocolor(255, 255, 255, alpha))
        end
    end
end
