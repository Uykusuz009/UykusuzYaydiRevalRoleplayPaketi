	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>1441) then
				...
				local targetPlayer, targetPlayerNick = exports.rl_global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					elseif targetCol ~= playerCol then
						...
						if (jailTimer) then
							...
							local targetPlayerhasmoney = exports.rl_global:getMoney(targetPlayer, true)
							local amount = tonumber(fine)
							if not exports.rl_global:takeMoney(targetPlayer, amount) then
								finebank = true
								exports.rl_global:takeMoney(targetPlayer, targetPlayerhasmoney)
							...
							if restrainedObj then
								...
								if restrainedObj == 45 then -- If handcuffs.. take the key
									...
									exports['rl_items']:deleteAll(47, dbid)
								end
								exports.rl_global:giveItem(thePlayer, restrainedObj, 1)
							...
							exports.rl_logs:logMessage("[PD/ARREST] " ..  factionRankTitle .. " " .. username .. " jailed " .. targetPlayerNick .. " for " .. jailtime .. " minute(s). Reason: " .. reason , 30)
						end
					end
				end
			end
...
-- function showJailtime(thePlayer, commandName)
	...
	-- if isJailed then
		...
		-- outputChatBox(">> #fffffPolis hapishanesinde bulunuyorsunuz " .. jailtime .. " süre dolduğunda çıkacaksınız.", thePlayer, 255, 194, 14)
	-- else
		-- outputChatBox(">> #fffffPolis hapishanesinde bulunmuyorsunuz.", thePlayer, 255, 0, 0,true)
	-- end
-- end
...
function jailRelease(thePlayer, commandName, targetPlayerNick)
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) or exports.rl_global:isPlayerAdmin1(thePlayer) and getElementData(thePlayer, "adminduty") == 1 then
			if not (targetPlayerNick) then
				...
				if targetPlayer then
					...
					if (jailTimer) then
						...
						timerPDUnjailPlayer(targetPlayer)
						exports.rl_logs:logMessage("[PD/RELEASE]" .. username .. " released " .. targetPlayerNick , 30)
					else
