...
function(player, cmd, speed)
    ...
    if vehicle then
        if (fID == 1) or (fID == 50) then
            if exports.rl_global:hasItem(vehicle, 284) then
                        ...
                        if isVerifiedElement(radar, "vehicle") and getElementData(radar, "anpr:state") then
                            ...
                            if hasVehicleANPR(hitElement) then
                                for seat,player in pairs(getVehicleOccupants(radar)) do
                                    ...
                                    setElementData(player, "anpr:vehicleName", exports.rl_global:getVehicleName(hitElement), true)
                                    setElementData(player, "anpr:vehicleName", exports.rl_global:getVehicleName(hitElement), true)
                                    setElementData(player, "anpr:vehicleDirection", direction, true)
                                    outputChatBox("[RADAR] A ANPR hit was recorded on a " .. getColorName(c1, c2) .. " " .. exports.rl_global:getVehicleName(hitElement) .. " with plates " .. getElementData(hitElement, "plate") .. ".", player, 255, 0, 0, false)
...
function hasVehicleANPR(theVehicle)
	if getElementType(theVehicle) == "vehicle" then
		local anpr = exports.rl_mdc:getANPRTable()
