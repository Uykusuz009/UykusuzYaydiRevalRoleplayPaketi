...
function setPlayerFreecamEnabled(player)
	if not isPlayerFreecamEnabled(player) then
		...
		return triggerClientEvent(player,"doSetFreecamEnabledTV", root, x,y,z, false)
	else
		return false
	end
end
...
function setPlayerFreecamDisabled(player)
	if isPlayerFreecamEnabled(player) then
		...
		return triggerClientEvent(player,"doSetFreecamDisabledTV", root, false)
	else
		return false
	end
end
...
function setPlayerFreecamOption(player, theOption, value)
	return triggerClientEvent(player,"doSetFreecamOptionTV", root, theOption, value)
end
...
function tv(player)
	local hasTV = exports.rl_global:hasItem(player, 104, 1)
...
addEvent("useTV", true)
addEventHandler("useTV", root, tv)

addCommandHandler("movetv", function(player)
...
addCommandHandler("starttv", function(player)
	if getElementData(player, "faction") == 20 then
		if not isTVRunning() then
			outputChatBox("[TV] " .. getPlayerName(player):gsub("_", " ") .. " started a TV Show. ((/tv to watch))", root, 200, 100, 200)
			watching = 0
			earnings = 0
...
addCommandHandler("endtv", function(player)
	if getElementData(player, "faction") == 20 then
		if isTVRunning() then
			setElementDimension(marker, 65535)
			outputChatBox("[TV] " .. getPlayerName(player):gsub("_", " ") .. " ended the TV Show.", root, 200, 100, 200)
			...
			for k, v in ipairs(getElementsByType("player")) do
				...
				if getElementData(v, "faction") == 20 then
					outputChatBox("[TV] Max. Viewers: " .. watching .. ", Earnings: $" .. exports.rl_global:formatMoney(earnings), v, 200, 100, 200)
				end
			end
		else
...
addCommandHandler("watchers", function(player)
	if getElementData(player, "faction") == 20 then
		if isTVRunning() then
			local count = 0
			for k, v in ipairs(getElementsByType("player")) do
			...
			outputChatBox("[TV] Max. Viewers: " .. watching .. ", Earnings: $" .. exports.rl_global:formatMoney(earnings), player, 200, 100, 200)
		else
			outputChatBox("There's no TV Show running.", player, 255, 0, 0)
		end
	end
...
function add(affectedPlayers)
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.rl_global:giveMoney(exports.rl_faction:getFactionFromID(20), LSN_MONEY * shownto)
	end
end