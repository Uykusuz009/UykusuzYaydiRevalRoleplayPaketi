...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 215, 202, "Silah Konum", false)
...
function populateWeaponGridList()
	for i = 1, 12 do
		...
		if weapon ~= 0 then
			...
			guiGridListSetItemText(weaponsGridList, row, 3, getElementData(localPlayer, "attachingWeapon" .. weapon) and "Evet" or "Hayır", false, false)
		end
	end
end
		...
		addEventHandler("onClientGUIChanged", posEditBox[i],
			function()
				...
				if math.abs(x) > 0.5 or math.abs(y) > 0.5 or math.abs(z) > 0.5 then
					...
					if isElement(editingObject) then
						exports.rl_bones:attachElementToBone(editingObject, localPlayer, bone, x, y, z, rx, ry, rz)
					end
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	exports.rl_bones:attachElementToBone(object, localPlayer, bone, x, y, z, rx, ry, rz)
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		if isElement(editingObject) then
			exports.rl_bones:attachElementToBone(editingObject,
...
addEventHandler("requestWeaponModel", root, function(weapon)
