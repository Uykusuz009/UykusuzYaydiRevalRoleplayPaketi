...
addEventHandler("onPlayerDamage", root, function(attacker, _, bodyPart, _)
	if (attacker) and (attacker ~= source) and (getElementType(attacker) == "player") and (bodyPart == 9) then
		triggerClientEvent(attacker, "weapon.playHeadshotSound", attacker)
	end
end)