...
addEventHandler("onClientResourceStart", resourceRoot, function()
	for neonName, replaceModel in pairs(availableNeons) do
		local neonCOL = engineLoadCOL("public/neons/neonCollision.col")
		local neonDFF = engineLoadDFF("public/neons/" .. neonName .. ".dff")
...
addEventHandler("neon.showList", root, function()
	...
	exports.rl_global:centerWindow(neonGUI)
	...
	image = guiCreateStaticImage(9, 210, 400, 606/3, "public/images/neon1.jpg", false, neonGUI)
	...
	ok = guiCreateButton(9, 460, 190, 32, "Satın Al (60 TL)", false, neonGUI)
	...
	addEventHandler("onClientGUIClick", root, function(b)
		if (b == "left") then
			if (source == gridlist) then
				...
				guiStaticImageLoadImage(image, "public/images/neon" .. convert_number[neonIndex] .. ".jpg")
			...
			elseif (source == ok) then
				...
				showCursor(false)
				destroyElement(neonGUI)
				triggerServerEvent("market.buyVehicleNeon", localPlayer, vehicleID, neonIndex, 60)
			end
		end
	end)
end)
