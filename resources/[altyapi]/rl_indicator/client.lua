...
addEventHandler("onClientPlayerDamage", localPlayer, function(attacker, weapon, bodypart, loss)
	if getElementData(localPlayer, "logged") and exports.rl_settings:getPlayerSetting(localPlayer, "damage_indicator_visible") then
		if attacker and getElementType(attacker) == "player" then
			if weapon and weapon > 9 then
				...
				if not attackTargetState then
					attackTargetState = true
					renderTimer = setTimer(renderAttackerCircle, 0, 0)
				end
			end
		end
	end
	...
	if getElementData(localPlayer, "logged") and exports.rl_settings:getPlayerSetting(localPlayer, "damage_indicator_visible") then
		...
		if delay <= (duration + fadeTime) then
			...
			if angle then
				...
				if x and y then
					dxDrawImage(x, y, width, height, "public/images/arrow.png", angle, -128, -32, tocolor(255, 255, 255, alpha))
				end
			end
		else
