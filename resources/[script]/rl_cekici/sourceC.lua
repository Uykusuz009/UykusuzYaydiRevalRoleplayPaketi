...
local RAMP_DEFAULT_POSITION = {0.61, -1, -0.15}
	...
	function (dataName, oldVal)
		...
		if dataName == "carAnimation" then
			...
			if dataVal then
				...
				local targetPosX, targetPosY, targetPosZ = getPositionFromElementOffset(carAnimation[source][1], 0, rampY - 1.5, rampZ)
				...
				if carAnimation[source][2] == "up" then
					...
					local flatbedX, flatbedY, flatbedZ = getPositionFromElementOffset(effect.towedBy, 0, -2.425, 0.2)
				...
				if carAnimation[source][2] == "down" then
					...
					local groundX, groundY, groundPreZ = getPositionFromElementOffset(effect.towedBy, 0, -10.5, minZ)
...
function findAttachOffsets(vehicle, towedBy)
	...
	return 0, -2.425, 0.1 + dist, 0, 0, frRotZ - toRotZ
end
				...
				for veh, state in pairs(rampState) do
					if isElement(veh) then
						if isElementOnScreen(veh) then
							-- Rámpa
							local x, y, z = getPositionFromElementOffset(veh, -1, -4, 0.25)
							...
							-- Forgattyú
							local x, y, z = getPositionFromElementOffset(veh, 0, 0.20, 0.5)
							local dist = getDistanceBetweenPoints3D(px, py, pz, x, y, z)
