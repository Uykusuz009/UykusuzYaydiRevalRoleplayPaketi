...
general.insertPersonel = function(self , element, record, reason, sourcePlayer)
    ...
    if type == "vehicle" then
        vehShopData = exports["rl_vehicle_manager"]:getInfoFromVehShopID(tonumber(dbid))
    end
    local name  = type == "player" and element.charactername or vehShopData.vehvehbrand.." "..vehShopData.vehmodel; --
                ...
                if rows > 0 then
                    ...
                    for k , v in pairs(res) do 
                        ...
                        if tonumber(v.deleted) == 0 then
                            ...
                            local vehShopData = exports["rl_vehicle_manager"]:getInfoFromVehShopID(tonumber(v.model))
                            ...
                            if vehShopData then 
                                vehbrand = vehShopData.vehvehbrand
                                model = vehShopData.vehmodel
                            else
                                vehbrand = getElementData(vehicle, "vehbrand")
								model = getElementData(vehicle, "model")
							end

                            triggerClientEvent(player , "mdc.execute" , player , "vm.tablist[1].vehicles.push({id : "..v.id.." , plate : '"..v.plate.."' , model : '"..model.."' , color : '"..tostring(color).."'}) ")
