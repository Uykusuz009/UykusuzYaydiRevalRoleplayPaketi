		...
		if (logged==1) then
			...
			if (factionType==2) or (factionType == 3) or getElementData(thePlayer,'faction') == 143 then
				if not (targetPlayerNick) or not (fonksiyon) or not (koltukID) then
					...
					local theVehicle = exports.rl_pool:getElement("vehicle", getElementData(thePlayer, "lastvehid"))
					...
					if targetPlayer then
						...
						if (distance<=10) then
							if not (distance2<=10) then
								outputChatBox("#575757Reval: #f0f0f0" .. targetPlayerName .. " isimli şahıs en son bindiğiniz araçtan uzakta.", thePlayer, 255, 0, 0, true)
								return
							end
							if getVehicleOccupant(theVehicle, tonumber(koltukID)) then
								outputChatBox("#575757Reval: #f0f0f0Kişiyi atmak istediğiniz koltuk şu an dolu.", thePlayer, 255, 0, 0, true)
								return
							end
							local vehicleID = getElementData(thePlayer, "lastvehid")
							outputChatBox("#575757Reval: #f0f0f0" .. targetPlayerName .. " isimli şahsı #" .. vehicleID .. " ID'li aracın '" .. koltukID .. "' ID'li koltuğuna bindirdiniz.", thePlayer, 0, 255, 0, true)
							outputChatBox("#575757Reval: #f0f0f0" .. getPlayerName(thePlayer) .. " isimli şahıs tarafından #" .. vehicleID .. " ID'li aracın '" .. koltukID .. "' ID'li koltuğuna bindirildiniz.", targetPlayer, 0, 255, 0, true)
