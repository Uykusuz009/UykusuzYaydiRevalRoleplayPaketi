...
function isPDDD2()
	return getElementData(getLocalPlayer(), "faction") == 1 or getElementData(getLocalPlayer(), "faction") == 143
end
		...
		if (getElementType(target)=="vehicle") then
			if (getPedWeapon(localPlayer)==24) then
				...
				if (mode==2 and getPedOccupedVehicle( localPlayer ) ~= target) then
					actualspeed = exports.rl_global:getVehicleVelocity(target)
					outputChatBox(exports.rl_global:getVehicleName(target) .. " markali aracin hizi " .. math.floor(actualspeed) .. " km/h.", 255, 194, 14)
				end
			end
		end
	end
