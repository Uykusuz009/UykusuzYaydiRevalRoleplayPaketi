...
function ticketPlayer(thePlayer, commandName, targetPlayerNick, amount, ...)
	...
	if (logged==1) then
		...
		if (factionID==1 or factionID==143) then
			if not (targetPlayerNick) or not (amount) or not (...) then
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						exports.rl_global:giveMoney( getTeamFromName("Government of Los Santos"), math.ceil(tax*amount) )
						...
						if takeFromBank > 0 then
							...
							----exports.rl_anticheat:changeProtectedElementDataEx(targetPlayer, "bankmoney", bankmoney - takeFromBank, false)
						end
					else
						outputChatBox("You are too far away from " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
					end
...
function takeLicense(thePlayer, commandName, targetPartialNick, licenseType, hours)
	...
	if (logged==1) then
		...
		if (ftype==2) or exports.rl_integration:isPlayerTrialAdmin(thePlayer) then
			if not (targetPartialNick) then
				...
				if not (licenseType) or not (hours) or hours < 0 or (hours > 10 and not exports.rl_integration:isPlayerTrialAdmin(thePlayer)) then
					outputChatBox("KULLANIM: /" .. commandName .. " [Player Partial Nick] [license Type 1:Driving 2:Weapon] [Hours]", thePlayer)
				else
					...
					if targetPlayer then
						...
						if (tonumber(licenseType)==1) then
							if(tonumber(getElementData(targetPlayer, "license.car")) == 1) then
								...
								----exports.rl_anticheat:changeProtectedElementDataEx(targetPlayer, "license.car", -hours, true)
							else
						...
						elseif (tonumber(licenseType)==2) then
							if(tonumber(getElementData(targetPlayer, "license.gun")) == 1) then
								...
								----exports.rl_anticheat:changeProtectedElementDataEx(targetPlayer, "license.gun", -hours, true)
							else
