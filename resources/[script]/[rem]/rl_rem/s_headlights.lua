...
function vehicleBlown()
	----exports.rl_anticheat:changeProtectedElementDataEx(source, "lspd:flashers", nil, true)
end
...
local function canUseStrobes(vehicle, data)
	if data then
		if data[2] and exports.rl_global:hasItem(vehicle, data[2]) then
			return data[1]
		end
		for i = 3, #data do
...
function toggleFlasherState()
	...
	if (theVehicle) then
		...
		if currentFlasherState ~= 0 then
			----exports.rl_anticheat:changeProtectedElementDataEx(theVehicle, "lspd:flashers", nil, true)
		else
			...
			if color then
				----exports.rl_anticheat:changeProtectedElementDataEx(theVehicle, "lspd:flashers", color, true)
			else
