function kimlik(thePlayer, commandName, targetPlayer)
	if not (targetPlayer) then
				outputChatBox("Kullanım: /" .. commandName .. " [Oyuncu Ad/ID] ", thePlayer, 255, 194, 14)
			else
				...
				if targetPlayer then
					...
					if not getElementData(thePlayer, "kimlik") == true then
						...
						local theVehicleT = getPedOccupiedVehicle(targetPlayer)
							...
							if distance < 3 then
	exports.rl_hud:sendBottomNotification(targetPlayer, "Aşağıdakiler '"..username.."' isimli kişinin kimlik bilgileridir.", "Adı ve Soyadı "..getPlayerName(thePlayer):gsub("_", " ").." | Cinsiyeti: "..cinsiyet.." (0 = Erkek, 1 = Kadın) | " )
