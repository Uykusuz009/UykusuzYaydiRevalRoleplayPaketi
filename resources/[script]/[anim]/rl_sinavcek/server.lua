...
addCommandHandler("sinav",
	...
	if veh then outputChatBox("[!]#FFFFFF Bu komutu araçta kullanamazsınız.", player, 255, 0, 0, true) return end
		if (not animEnable[player]) then
			...
			triggerClientEvent(syncPlayers, "sinav", player, true)
			outputChatBox("[!]#FFFFFF Şınav çekiyorsunuz. Bitirmek için komutu tekrarlayabilirsiniz.", player, 0, 255, 0, true)
		else
			animEnable[player] = false
			triggerClientEvent(syncPlayers, "sinav", player, false)
			outputChatBox("[!]#FFFFFF Artık şınav çekmiyorsunuz. Eğer bugda kaldıysanız komutu tekrarlayın.", player, 255, 0, 0, true)
		end
	end
)
...
addEventHandler("onClientSync", resourceRoot,
    function()
		...
		for player, enable in ipairs(animEnable) do
			if (enable) then
				triggerClientEvent(client, "sinav", player, true)
			end
		end
    end 
)
