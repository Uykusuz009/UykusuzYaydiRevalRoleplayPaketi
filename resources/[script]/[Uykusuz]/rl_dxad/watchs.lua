...
function stopWatch(thePlayer, commandName)
	if watcher[thePlayer] and watched[watcher[thePlayer]] then
		...
		if watchTimers[thePlayer] and isTimer(watchTimers[thePlayer]) then
			killTimer(watchTimers[thePlayer])
			watchTimers[thePlayer] = nil
		end
	...
	if exports.rl_integration:isPlayerLeaderAdmin(thePlayer) then
		if targetPlayer then
			...
			if targetPlayer then
				...
				exports.rl_logs:addLog("watch", exports.rl_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili " .. targetPlayerName .. " isimli oyuncuyu izlemeye başladı.")
			end
		else
			if watcher[thePlayer] and watched[watcher[thePlayer]] then
				for i, k in pairs(watched[watcher[thePlayer]]) do
				...
				triggerClientEvent(thePlayer, "stopScreen", thePlayer)
				outputChatBox("[!]#FFFFFF Artık kimseyi izlemiyorsunuz.", thePlayer, 255, 0, 0, true)
				exports.rl_logs:addLog("watch", exports.rl_global:getPlayerFullAdminTitle(thePlayer) .. " isimli yetkili izlemeyi bıraktı.")
			else
				triggerClientEvent(thePlayer, "stopScreen", thePlayer)
...
addEvent("startWatchingPlayer", true)
addEventHandler("startWatchingPlayer", root, function(targetPlayer)
	if client and targetPlayer then
		if exports.rl_integration:isPlayerLeaderAdmin(client) then
			if watcher[client] and watched[watcher[client]] then
				for i, k in pairs(watched[watcher[client]]) do
					if k == client then
						watched[watcher[client]][i] = nil
					end
				end
			end
			watcher[client] = targetPlayer
			if not watched[targetPlayer] then
				watched[targetPlayer] = {}
			end
			table.insert(watched[targetPlayer], client)
			outputChatBox("[!]#FFFFFF " .. getPlayerName(targetPlayer):gsub("_", " ") .. " isimli oyuncuyu izliyorsunuz.", client, 0, 255, 0, true)
			saveLog("watch", 
...
addEvent("addWatchLog", true)
addEventHandler("addWatchLog", root, function(adminName, targetName)
	if client then
		exports.rl_logs:addLog("watch", exports.rl_global:getPlayerFullAdminTitle(client) .. " isimli yetkili " .. targetName .. " isimli oyuncuyu izlemeye başladı.")
	end
end)
addEvent("stopWatchingPlayer", true)
addEventHandler("stopWatchingPlayer", root, function()
	if client then
		stopWatch(client)
		saveLog("watch", 
			getPlayerName(client), 
