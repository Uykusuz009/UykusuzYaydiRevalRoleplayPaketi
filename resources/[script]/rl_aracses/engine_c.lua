...
-- burdan ses seviyelerini ayarlaya bilirsiniz.
ENGINE_ENABLED = true
ENGINE_VOLUME_MASTER = 0.12 -- volume multiplier
...
-- Aşağıdaki bölüm egzoz patlama efektini ekleme yeridir.
-- İsteğe bağlı araca ekleye bilirsiniz.


local als = {
	[602] = { 
		{0.7, -2.2, -0.48}, -- tek taraflı patlama
		{-0.7, -2.2, -0.48}, -- çift taraflı patlama 
		{0.58, -2.2, -0.48},
		{-0.58, -2.2, -0.48},
	},
	[436] = { 
		...
		if isElement(vehicle) then
			...
			if engine then
				...
				if getVehicleEngineState(vehicle) == true and distance < ENGINE_SOUND_DISTANCE*2 then
					...
					local isSkidding = controller and ( ( getPedControlState(controller, "accelerate") and getPedControlState(controller, "brake_reverse") or getPedControlState(controller, "handbrake") ) and velocity < 1 ) or false
