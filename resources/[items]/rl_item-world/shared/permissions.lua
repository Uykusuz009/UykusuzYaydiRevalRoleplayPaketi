...
items = exports['rl_items']
itemtexture = exports['rl_item-texture']
...
function canEditItemProperties(thePlayer, object)
	...
	if exports.rl_global:hasItem(localPlayer, 4, interiorID) or exports.rl_global:hasItem(localPlayer, 5, interiorID) or (exports.rl_integration:isPlayerTrialAdmin(thePlayer) and exports.rl_global:isAdminOnDuty(thePlayer)) or exports.rl_integration:isPlayerDeveloper(thePlayer) then
		return true
	end
	return false
end
					...
					if isNumeric(theItem[1]) then --if number, then treat it as a itemID
						...
						if hasItem then
							...
							if theItem[2] then
								...
								--outputDebugString(tostring(exports['rl_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])))
								...
								--if exports["rl_items"]:isBadge(theItem[1]) then
									--if (getElementData(thePlayer, badges[theItem[1]][1])) then
									--if exports["rl_items"]:isWearingBadge(thePlayer, theItem[1]) then
									--	countOr = countOr + 1
							...
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										if exports.rl_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2])) then
											countOr = countOr + 1
										end
									else
										if exports.rl_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1])) then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.rl_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										if exports.rl_factions:isPlayerInFaction(thePlayer, factionID) then
											countOr = countOr + 1
										end
									end
								end
						...
						elseif textFunction == "FL" or textFunction == "FACTIONLEADER" then --check for faction leadership
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										local isMember, rank, isLeader = exports.rl_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									else
										local isMember, rank, isLeader = exports.rl_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.rl_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										local isMember, rank, isLeader = exports.rl_factions:isPlayerInFaction(thePlayer, factionID)
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
