...
function getItemPropertiesData(object)
	...
	if creator > 0 then
		creatorName = exports.rl_global:getCharacterNameFromID(creator)
	end
...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("rl_item-world")) then
		...
		if id then
			...
			local itemName = exports['rl_items']:getItemName(tonumber(getElementData(object, "itemID")), getElementData(object, "itemValue"))
