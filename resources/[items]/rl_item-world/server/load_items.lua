			...
			if rows > 0 then
				for index, row in ipairs(res) do
					...
					if itemID < 0 then -- weapon
						...
						local modelid = exports['rl_items']:getItemModel(itemID, itemValue)
						...
						local rx, ry, rz, zoffset = exports['rl_items']:getItemRotInfo(itemID)
						local obj = createItem(id, itemID, itemValue, modelid, x, y, z + (zoffset or 0), rx+rx2, ry+ry2, rz+rz2)
						
						if isElement(obj) then
							exports.rl_pool:allocateElement(obj, itemID, true)
							setElementDimension(obj, dimension)
							setElementInterior(obj, interior)
							setElementData(obj, "creator", creator)
							setElementData(obj, "createdDate", createdDate)
...
function loadWorldItems(res)
	...
	dbExec(exports.rl_mysql:getConnection(), "DELETE FROM `worlditems` WHERE `protected`='0' AND `itemID` NOT IN(81, 103, 169) AND ((DATEDIFF(NOW(), creationdate) > 30) OR (DATEDIFF(NOW(), creationdate) > 7 AND `itemID` = 72)) ")
	...
	setTimer(restartResource, 5000, 1, getResourceFromName("rl_item-texture"))
end
