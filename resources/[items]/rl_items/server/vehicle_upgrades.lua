function addUpgrade(source, vehicle, itemSlot, itemID, itemValue)
	...
	if takeItemFromSlot(source, itemSlot) then
		if addVehicleUpgrade(vehicle, itemValue) then
			...
			exports.rl_global:sendLocalMeAction(source, "adds " .. getItemDescription(itemID, itemValue) .. " to their " .. exports.rl_global:getVehicleName(vehicle) .. "(" .. getVehicleName(vehicle) .. ").")
			exports.rl_save:saveVehicleMods(vehicle)
			...
			outputChatBox("This upgrade is not compatible to this " .. exports.rl_global:getVehicleName(vehicle) .. ".", source, 255, 194, 14)
...
function moveUpgradeFromElement(upgrade)
	...
	if hasSpaceForItem(client, 114, upgrade) then
		if removeVehicleUpgrade(source, upgrade) then
			...
			exports.rl_save:saveVehicleMods(source)
		else
