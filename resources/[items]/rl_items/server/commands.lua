...
function breathTest(thePlayer, commandName, targetPlayer)
	if (exports.rl_global:hasItem(thePlayer, 53)) then
...
function givePlayerBadge(thePlayer, commandName, targetPlayer, ...)
	...
	if not (tonumber(leader)==1) then -- If the player is not the leader
		...
		if not targetPlayer or not badgeNumber then
			outputChatBox("KULLANIM: /" .. commandName .. " [Karakter Adı / ID] [Badge Number]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.rl_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then -- is the player online?
				local targetPlayerName = targetPlayerName:gsub("_", " ")
				local logged = getElementData(targetPlayer, "logged")
				if (not logged) then -- Are they logged in?
					outputChatBox("Player is not logged in.", thePlayer, 255, 0, 0)
				else
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>4) then -- Are they standing next to each other?
						...
						exports.rl_global:sendLocalMeAction(thePlayer, "issues " .. targetPlayerName .. " " .. badge[2] .. ", reading " .. badgeNumber .. ".")
						exports.rl_global:giveItem(targetPlayer, itemID, prefix .. badgeNumber)
					end
				end
			end
		end
...
function writeNote(thePlayer, commandName, ...)
	if true then
		outputChatBox("[!]#FFFFFF Bu sistem aktif değil.", thePlayer, 255, 0, 0, true)
	...
	elseif getElementData(thePlayer, 'note-timeout') and math.abs(getElementData(thePlayer, 'note-timeout') - tick) < 5000 then
		...
		exports.rl_global:sendLocalMeAction(thePlayer, "writes a note on a piece of paper.")
...
function changeLock(thePlayer)
	if exports.rl_integration:isPlayerDeveloper(thePlayer) then
		...
		if theVehicle then
			...
			if dbid > 0 then
				exports['rl_vehicle-manager']:addVehicleLogs(dbid, 'changelock', thePlayer)
			...
			local dbid, entrance, exit, interiortype = exports['rl_interior']:findProperty(thePlayer)
			if dbid > 0 then
				if interiortype == 2 then
					...
					exports['rl_interior-manager']:addInteriorLogs(dbid, 'changelock', thePlayer)
...
addEventHandler("items.searchPlayer", root, function(player)
	...
	triggerEvent("subscribeToInventoryChanges", client, player)
	triggerClientEvent(client, "showInventory", client, player)
end)